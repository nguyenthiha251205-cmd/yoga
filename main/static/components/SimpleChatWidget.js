...
async function sendMessage(message) {
    ...
    try {
        ...
        if (response.ok) {
            ...
            // Append the sent message immediately (no need to reload)
            if (data.message) {
...
// Append a single message to the chat (old function - kept for compatibility)
...
// Append message to UI only (without modifying state)
function appendMessageToUI(messageData) {
    console.log('SimpleChatWidget: Appending message to UI:', messageData);
    ...
    console.log('SimpleChatWidget: Message appended to UI successfully');
}
    ...
    chatPollingInterval = setInterval(async () => {
        try {
            ...
            if (data.messages && data.messages.length > 0) {
                ...
                const lastMessageId = chatState.messages.length > 0 
                ...
                if (newMessages.length > 0) {
                    console.log('SimpleChatWidget: New messages detected, appending...', newMessages.length);
